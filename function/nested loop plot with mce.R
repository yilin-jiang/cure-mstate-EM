...
ggplot_nlp_mce <- function(dat,
  ...
  mce_low  <- min(dat[[rlang::as_name(estim)]] - 1.96 * dat[[rlang::as_name(mce)]], na.rm = TRUE)
  mce_high <- max(dat[[rlang::as_name(estim)]] + 1.96 *dat[[rlang::as_name(mce)]], na.rm = TRUE)
  ylim_low <- min(
  ...
  p <- dat_obj$df_main %>% 
    ...
    ggplot2::scale_x_continuous(name = "Scenarios", breaks = gridline_seq, labels = NULL,  expand = c(0, 0)) 
