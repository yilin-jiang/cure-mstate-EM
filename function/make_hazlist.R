make_hazlist=function(cox_full,tran.index,data_cov,time_list,tran_c,curecol){
  #tran_c is the transition list corresponding to cured status
  #curecol is the corresponding cure column names based on tran_c list
  data=data_cov
  if(tran.index %in% tran_c){
    curecolname=curecol[which(tran_c==tran.index)]
    data[,curecolname]=1}
